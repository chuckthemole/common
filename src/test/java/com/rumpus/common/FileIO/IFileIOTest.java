...
/**
 * Test class for the IFileReader interface. Add tests for the
 * readModelsFromFile method.
 *
 * TODO: this will need to be done for xml when it is implemented. Right now it
 * is only implemented for JSON.
 */
class IFileIOTest {
    ...
    void testReadModelsFromFile_Success() {
        ...
        try (MockedStatic<FileIOUtil> readerMock = mockStatic(FileIOUtil.class)) {
            readerMock.when(() -> FileIOUtil.readFileAsString(IFileIOTest.JSON_USERS_FILE))
                    .thenReturn(IFileIOTest.jsonContent);
    ...
    void testReadModelsFromFile_EmptyFile() {
        ...
        try (MockedStatic<FileIOUtil> readerMock = mockStatic(FileIOUtil.class)) {
            readerMock.when(() -> FileIOUtil.readFileAsString(IFileIOTest.JSON_USERS_FILE_EMPTY))
                    .thenReturn(IFileIOTest.emptyJsonContent);
    ...
    void testReadModelsFromFile_ParsingError() {
        ...
        // Mocking the Reader.readFileAsString method
        try (MockedStatic<FileIOUtil> readerMock = mockStatic(FileIOUtil.class)) {
            readerMock.when(() -> FileIOUtil.readFileAsString(IFileIOTest.JSON_USERS_FILE_INVALID))
                    .thenReturn(IFileIOTest.invalidJsonContent);
    ...
    void testWriteModelsToFile_Success() {
        ...
        // boolean result = this.jsonFileReader.writeModelsToFile("output.json",
        // models);
        // assertTrue(result);
        assertTrue(true); // TODO: implement this test. Still need to implement the
                          // writeModelsToFile method.
    }
