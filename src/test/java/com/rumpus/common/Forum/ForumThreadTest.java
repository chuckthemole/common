...
/**
 * Tests {@link ForumThread} and in turn {@link AbstractGraph} since the
...
public class ForumThreadTest extends CommonForumTest {
    ...
    void testNextAndPrevioud() {
        ...
        CommonForumTest.thread1.previous().previous().previous().previous(); // move back too far,
                                                                             // should make current
                                                                             // null
        assertEquals(null,CommonForumTest.thread1.getCurrent());
        CommonForumTest.thread1.setCurrent(CommonForumTest.thread1.getHead()); // set head back to
                                                                               // current
        CommonForumTest.thread1.next().next().next().next().next().next(); // move back up far,
                                                                           // should make current
                                                                           // null
        assertEquals(null,CommonForumTest.thread1.getCurrent());
    }
    ...
    void testChildren() {
        this.addThreeNodesToSequenceInThread1();
        assertEquals(false,CommonForumTest.thread1.hasChildren()); // expect that current node has
                                                                   // no children
        CommonForumTest.thread1.next();
        assertEquals(false,CommonForumTest.thread1.hasChildren()); // expect that current node has
                                                                   // no children
        CommonForumTest.thread1.addChildrenToCurrentNode(CommonForumTest.expectedChildren);
        assertEquals(true,CommonForumTest.thread1.hasChildren()); // expect that current node has
                                                                  // children
        // test children
        assertEquals(CommonForumTest.expectedChildren.get(0),
                CommonForumTest.thread1.child().getCurrent());
        assertEquals(CommonForumTest.expectedChildren.get(1),
                CommonForumTest.thread1.next().getCurrent());
        assertEquals(CommonForumTest.expectedChildren.get(2),
                CommonForumTest.thread1.next().getCurrent());
        assertEquals(CommonForumTest.expectedChildren.get(3),
                CommonForumTest.thread1.next().getCurrent());
    }
    ...
    private void addThreeNodesToSequenceInThread1() {
        CommonForumTest.thread1
                .addToSequence(ForumPostNode.createNodeFromForumPost(CommonForumTest.post2));
        CommonForumTest.thread1
                .addToSequence(ForumPostNode.createNodeFromForumPost(CommonForumTest.post3));
        CommonForumTest.thread1
                .addToSequence(ForumPostNode.createNodeFromForumPost(CommonForumTest.post4));
    }

    private void addTwoNodesToSequenceInThread2() {
        CommonForumTest.thread2
                .addToSequence(ForumPostNode.createNodeFromForumPost(CommonForumTest.post1));
        CommonForumTest.thread2
                .addToSequence(ForumPostNode.createNodeFromForumPost(CommonForumTest.post3));
    }

    private void initThenAddOneNodeToSequenceInThread3() {
        CommonForumTest.thread3.init(ForumPostNode.createNodeFromForumPost(CommonForumTest.post3),
                TEST_PAGE_ID3);
        CommonForumTest.thread3
                .addToSequence(ForumPostNode.createNodeFromForumPost(CommonForumTest.post2));
    }

    private void testAddToSequenceThread1() {
        ForumPostNode current = CommonForumTest.thread1.getCurrent(); // current node
        assertNotEquals("Error: ForumPostNode current equal to null",null,current); // should not be
                                                                                    // equal to null
        int count = 0; // keep track of size
        while (current != null) { // check each node for correctness

            // build message
            final String message = StringUtil.buildStringFromArgs(
                    "Error with next: \n~ ~ ~ Current ~ ~ ~\n",
                    current.toString(),
                    "\n- - Next - - \n",
            ...
            if (count == 0) {
                assertEquals(CommonForumTest.userId2,current.getNext().getData().getUserId(),
                        message);
                ...
                assertEquals(CommonForumTest.userId3,current.getNext().getData().getUserId(),
                        message);
                ...
                assertEquals(CommonForumTest.userId4,current.getNext().getData().getUserId(),
                        message);
