...
abstract public class AbstractTile extends AbstractComponent {
    ...
    public abstract class AbstractTileComponentPart extends AbstractComponentPart {
        public AbstractTileComponentPart(AbstractComponentPart.ComponentPartType partType,
                String body) {
            super(partType, body);
        }
     ...
     *            the tile components, ie title and subtitle, as a String separated
     *            by the default delimiter. Example: "title,subtitle" the default
     *            delimiter is ",".
     */
    public AbstractTile(
    ...
    public void setChildrenForComponent() {
        ...
        final String[] tileTypeAndComponentArray = super.componentAsString
                .split(TILE_TYPE_DELIMITER);
        LOG(tileTypeAndComponentArray);
        if (tileTypeAndComponentArray.length > 0) {
            final TileType extractedTileType = TileType
                    .fromString(tileTypeAndComponentArray[0].strip());
            if (extractedTileType == TileType.ANCESTOR) {
                this.setHtmlTagType(AbstractHtmlObject.HtmlTagType.DIV);
                this.setHtmlAttributes(
                        super.componentAttributeManager.get(AbstractTile.ANCESTOR_ATTRIBUTE));
            } else if (extractedTileType == TileType.PARENT) {
                this.setHtmlTagType(AbstractHtmlObject.HtmlTagType.DIV);
                this.setHtmlAttributes(
                        super.componentAttributeManager.get(AbstractTile.PARENT_ATTRIBUTE));
            } else if (extractedTileType == TileType.CHILD) {
                this.setHtmlTagType(AbstractHtmlObject.HtmlTagType.DIV);
                this.setHtmlAttributes(
                        super.componentAttributeManager.get(AbstractTile.CHILD_ATTRIBUTE));
                ...
                this.setHtmlTagType(AbstractHtmlObject.HtmlTagType.DIV);
                this.setHtmlAttributes(
                        super.componentAttributeManager.get(AbstractTile.CONTAINER_ATTRIBUTE));
            ...
            LOG("tileTypeAndComponentArray.length is not greater than 0. Length: ",
                    String.valueOf(tileTypeAndComponentArray.length),
                    " Consider looking at delimiter.");
        }
    }
    ...
    private java.util.List<AbstractHtmlObject> createChildBody() {
        ...
        for (int tileComponentsArrayIndex = 0; tileComponentsArrayIndex < tileComponentsArray.length; tileComponentsArrayIndex++) {
            ...
            final String[] tileComponentAndLink = tileComponentsArray[tileComponentsArrayIndex]
                    .split(AbstractComponent.DEFAULT_LINK_DELIMITER);
            ...
            if (tileComponentAndLink.length == 1) {
                if (tileComponentsArrayIndex == 0) { // this is a title, not a subtitle
                    String[] typeAndTitleArray = tileComponentAndLink[0]
                            .split(AbstractTile.TILE_TYPE_DELIMITER);
                    if (typeAndTitleArray.length != 2) {
                        LOG("typeAndTitleArray.length is not 2. Length: ",
                                String.valueOf(typeAndTitleArray.length),
                                " Consider looking at delimiter.");
                        continue;
                    }
                    htmlObject = AbstractTileComponentPart
                            .createTitle(typeAndTitleArray[1].strip());
                    ...
                    htmlObject = AbstractTileComponentPart
                            .createTitle(tileComponentAndLink[0].strip());
                    htmlObject.setHtmlAttributes(
                ...
                if (tileComponentsArrayIndex == 0) { // this is a title, not a subtitle
                    ...
                    htmlObject = AbstractTileComponentPart.createLink(typeAndTitleArray[1].strip(),
                            tileComponentAndLink[1].strip());
                    htmlObject.setHtmlAttributes(
                            super.componentAttributeManager.get(AbstractTile.TITLE_ATTRIBUTE));
                } else { // this is a subtitle, not a title
                ...
                LOG("titleComponentAndLink.length is not 1 or 2. Length: ",
                        String.valueOf(tileComponentAndLink.length),
                        " Consider looking at delimiter.");
                continue;
            }
