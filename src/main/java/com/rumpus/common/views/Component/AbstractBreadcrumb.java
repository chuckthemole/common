...
public abstract class AbstractBreadcrumb extends AbstractComponent {
    ...
    public abstract class AbstractBreadcrumbComponentPart extends AbstractComponentPart {
        ...
        public AbstractBreadcrumbComponentPart(ComponentPartType breadcrumbComponentType,
                String body) {
    ...
    @Override
    public void setChildrenForComponent() {
        com.rumpus.common.views.Html.AbstractHtmlObject breadcrumbList = AbstractBreadcrumbComponentPart
                .createBreadcrumbList(); // breadcrumb list
        ...
        for (int crumbArrayIndex = 0; crumbArrayIndex < crumbArray.length; crumbArrayIndex++) {
            String[] crumbAndLink = crumbArray[crumbArrayIndex]
                    .split(AbstractComponent.DEFAULT_LINK_DELIMITER);
            AbstractHtmlObject htmlObject = null;
            if (crumbAndLink.length == 1) { // crumbs without links // TODO: maybe add a container
                                            // for spacing. rendering looks a little weird // TODO
                                            // update maybe set as link and use is-active for active
                htmlObject = AbstractBreadcrumbComponentPart
        ...
        this.setHtmlAttributes(
                super.componentAttributeManager.get(AbstractBreadcrumb.NAV_ATTRIBUTES));
    }
}
