...
/**
 * note: the member manager should be operated on before doing super operations.
...
public class ApiDBJdbcUsers<
    ...
    public ApiDBJdbcUsers(
        ...
        this.manager.setJdbcTemplate(CommonJdbc.getInstance().getJdbcTemplate()); // TODO: may not
                                                                                  // need this. look
                                                                                  // into it.
    ...
    public List<USER> getAll() {
        ...
        if (users != null && !users.isEmpty()) {
            users.stream().forEach((user) -> {
                // CommonUserDetails details = new
                // CommonUserDetails(this.manager.loadUserByUsername(user.getUsername()));
                final UserDetails userDetails = this.manager.loadUserByUsername(user.getUsername());
                final CommonUserDetails details = CommonUserDetails
    ...
    // TODO: this needs to be tested more. I don't think it was working correctly
    // last I tried. - chuck 2023/6/29
    // private byte[] serializeUserMetaWithClassSerializer(META meta) {
    // LogBuilder.logBuilderFromStringArgs("ApiDBJdbcUsers::serializeUserMetaWithClassSerializer()",
    // meta.toString()).info();
    // ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
    // try {
    // meta.serialize(meta, byteArrayOutputStream);
    // } catch (IOException e) {
    // LogBuilder.logBuilderFromStackTraceElementArray(e.getMessage(),
    ...
    public USER update(String id, USER newUser) {
        ...
        if (this.manager.userExists(username)) {
            if (username.equals(newUsername)) { // if we don't have to update username (PK) then go
                                                // through manager
    ...
    private void setDefaultQueries() {
        ...
        this.manager.setDeleteGroupAuthoritiesSql(
