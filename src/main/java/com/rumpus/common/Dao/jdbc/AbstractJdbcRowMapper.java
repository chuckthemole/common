 ...
 *            the model to map to
 */
abstract public class AbstractJdbcRowMapper<
        MODEL extends AbstractModel<MODEL, ?>> extends AbstractCommonObject
        implements
            RowMapper<MODEL> {

    /**
     * The function to map the result set to the model. The input is a pair of the
     * result set and the row number. The output is the model.
     */
    ...
    // Initialize AbstractJdbcRowMapper. This will create a function that returns
    // null if initMapperFunction() returns null.
    private void init() {
    ...
    /**
     * Initialize and return the map function. this function should be overridden by
     * the subclass.
     *
     * @return the row mapper function as a {@link Function} of
     *         {@link Pair}<ResultSet, Integer> to MODEL
     */
    protected abstract Function<Pair<ResultSet, Integer>, MODEL> initMapperFunction();

    /**
     * Set the map function. This will create a function that returns null if the
     * input is null.
     *
     * @param mapFunction
     *            the map function to set
     */
    public void setMapFunc(Function<Pair<ResultSet, Integer>, MODEL> mapFunction) {
        if (mapFunction == null) { // If the map function is null, set it to a function that returns
                                   // null
