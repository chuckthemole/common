...
/**
 * Singleton wrapper class around {@link JdbcTemplate} and
 * {@link NamedParameterJdbcTemplate}.
 *
 * This class provides a centralized management of the `JdbcTemplate` and
 * `NamedParameterJdbcTemplate` objects, ensuring that only one instance of
 * these objects exists during runtime, improving resource management and
 * maintaining a clean architecture.
 *
 * Key features: - Provides a thread-safe singleton implementation using
 * double-checked locking. - Simplifies JDBC operations by managing
 * `JdbcTemplate` and `NamedParameterJdbcTemplate` objects. - Ensures that the
 * `DataSource` is set once and shared across the application.
 */
final public class CommonJdbc extends AbstractCommonObject {
    ...
    private class CommonSimpleJdbc implements ICommon {
        ...
        private static Map<String, ?> simpleExecuteCall(Map<String, ?> parameters) {
            ...
            return new SimpleJdbcCall(CommonJdbc.commonJdbcSingleton.getJdbcTemplate())
    ...
    /**
     * The singleton instance of CommonJdbc. The singleton instance, marked as
    ...
    /**
     * Private constructor to prevent instantiation from outside. This ensures that
     ...
     */
    public static CommonJdbc getInstance() {
        return CommonJdbc.commonJdbcSingleton;
    }

    /**
     * Method to retrieve the singleton instance and set the DataSource. This method
     * uses double-checked locking to ensure thread-safety while avoiding
    ...
    /**
     * Query method to execute a SQL query and map the results to a list of objects
     * using a RowMapper.
     *
     * @param <T>
    ...
    /**
     * Query method to execute a SQL query and map the results to a single object
     * using a RowMapper.
     *
     * @param <T>
     *            the type of the object to be returned.
    ...
    /**
     * Executes an SQL update statement using a custom
     * {@link PreparedStatementCreator}.
     *
     * <p>
     * This overload should be used when bind parameters require explicit JDBC
     * handling, such as {@link java.sql.Blob}, binary data, streams, arrays, or
     * other database specific types that cannot be reliably inferred by
     * {@link JdbcTemplate}.
     * </p>
     *
     * @param preparedStatementCreator
     *            callback used to create and populate the
    ...
    /**
     * Update method to execute an SQL update statement and return the generated
     * keys.
     *
     * @param preparedStatementCreator
     ...
     * SimpleJdbcInsert *
     * ----------------------------------------------------------------------------
     * Purpose: These methods are used to insert data into the database using * the
