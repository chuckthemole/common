...
public class ForumPostNode extends AbstractNode<ForumPost, ForumPostNode> {
    ...
    // not equal: ", this.data.toString(), " not equal to ",
    // node.data.toString()).info();
    // isEqual = false;
    // }
    // if(!this.next.equals(node.next)) {
    // LogBuilder.logBuilderFromStringArgs("\nAbstractNode's are not equal:\n next
    // not equal: ", this.next.toString(), " not equal to ",
    // node.next.toString()).info();
    // isEqual = false;
    // }
    // if(!this.previous.equals(node.previous)) {
    // LogBuilder.logBuilderFromStringArgs("\nAbstractNode's are not equal:\n
    // previous not equal: ", this.previous.toString(), " not equal to ",
    // node.previous.toString()).info();
    // isEqual = false;
    // }
    // if(!this.headChild.equals(node.headChild)) {
    // LogBuilder.logBuilderFromStringArgs("\nAbstractNode's are not equal:\n
    // headChild not equal: ", this.headChild.toString(), " not equal to ",
    // node.headChild.toString()).info();
    // isEqual = false;
    // }
