...
/**
 * Class to keep track of collection's unique ids. Each object's set is
 * identified by its NAME
 *
 * TODO: this class can be tested and looked at more closely. What happens when
 * we remove an id? can we remove an id? etc.
 *
 */
public abstract class AbstractUniqueIdManager extends AbstractCommonManager<String, IdSet> {

    private static Map<String, IdSet> uniqueIds; // map of unique ids. holds name of unique id set
                                                 // and IdSet (size and set)

    public AbstractUniqueIdManager() {
        super(false);
     ...
     *
     * @param name
     *            the name of the set of ids to add to
